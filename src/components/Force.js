...
export class Force extends React.Component {
  ...
  centerOfGravityForGroups = (categories) => {
    ...
    return [ {x: firstCenter, y: yVal, group: groupOne} ,{x: secondCenter, y: yVal, group: groupTwo}]
  };
  ...
  updateLabels = () => {
    ...
    const labelSelection = d3.select('svg').selectAll('text').data(updatedCategoriesCenter);
    ...
    render() {
      ...
      return (
        <section className="page-excl-nav">
             <h1 className="graph-title"> FORCE</h1>
        <select className="force-select" onChange={e=> this.redraw(e.target.value)}>
          {selectCategories.map(c=> (<option key={c} value={c}>{c}</option>))}
        </select>
        <svg></svg>
        </section>
      );
